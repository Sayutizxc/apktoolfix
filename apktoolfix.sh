...
echo '
        ...
        |_______|__| |___._|___  |_______|___._|___  |__.__|______|
                           |_____|             |_____|             
'                                                                     
echo -e "${YELLOW}                           Apktool-Fix Version ${BLUE}2.7.0${RESTORE}\n"
...
APKTOOL_UPGRADE() {
    (wget -O /usr/bin/apktool https://raw.githubusercontent.com/iBotPeaches/Apktool/master/scripts/linux/apktool &>/dev/null;
     wget -O /usr/bin/apktool.jar https://bitbucket.org/iBotPeaches/apktool/downloads/apktool_2.7.0.jar &>/dev/null;
     [[ -e /usr/bin/apktool ]] && [[ -e /usr/bin/apktool.jar ]] && chmod +x /usr/bin/apktool /usr/bin/apktool.jar) &
    echo
    PROG_MESSAGE="${YELLOW}Installing Apktool 2.7.0${RESTORE}"
    COMP_MESSAGE="${YELLOW}Installed Apktool 2.7.0${RESTORE}"
    spinLoader
